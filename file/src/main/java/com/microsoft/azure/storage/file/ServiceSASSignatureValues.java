...
public final class ServiceSASSignatureValues {
    ...
    /**
     * Please refer to either {@link ShareSASPermission} or {@link FileSASPermission} depending on the resource
     ...
     * The name of the share the SAS user may access.
     */
    public ServiceSASSignatureValues withShareName(String shareName) {
        this.shareName = shareName;
        return this;
    }

    /**
     * The name of the filePath the SAS user may access.
     */
     ...
     * The name of the filePath the SAS user may access.
     */
    public ServiceSASSignatureValues withFilePath(String filePath) {
        this.filePath = filePath;
        return this;
    }

    /**
     * The name of the access policy on the share this SAS references if any. Please see
    ...
    public SASQueryParameters generateSASQueryParameters(SharedKeyCredentials sharedKeyCredentials) {
        ...
        Utility.assertNotNull("shareName", this.shareName);

        String resource = "s";
        String verifiedPermissions = null;
        // Calling parse and toString guarantees the proper ordering and throws on invalid characters.
        if (Utility.isNullOrEmpty(this.filePath)) {
            if (this.permissions != null) {
                verifiedPermissions = ShareSASPermission.parse(this.permissions).toString();
            }
        } else {
            if (this.permissions != null) {
                verifiedPermissions = FileSASPermission.parse(this.permissions).toString();
            }
            resource = "f";
        }
    ...
    private String getCanonicalName(String accountName) {
        ...
        */
        StringBuilder canonicalName = new StringBuilder("/file");
        canonicalName.append('/').append(accountName).append('/').append(this.shareName);

        if (!Utility.isNullOrEmpty(this.filePath)) {
            canonicalName.append("/").append(this.filePath);
        }
