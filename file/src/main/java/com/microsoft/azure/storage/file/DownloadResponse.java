...
/**
 * {@code DownloadResponse} wraps the protocol-layer response from {@link FileURL#download(FileRange, boolean, Context)}
 * to automatically retry failed reads from the body as appropriate. If the download is interrupted, the
 * {@code DownloadResponse} will make a request to resume the download from where it left off, allowing the user to
 * consume the data as one continuous stream, for any interruptions are hidden. The retry behavior is defined by the
 * options passed to the {@link #body(ReliableDownloadOptions)}. The download will also lock on the file's etag to
 * ensure consistency.
 * <p>
...
public final class DownloadResponse {
    ...
    private final RestResponse<FileDownloadHeaders, Flowable<ByteBuffer>> rawResponse;

    private final Function<HTTPGetterInfo, Single<DownloadResponse>> getter;

    public DownloadResponse(RestResponse<FileDownloadHeaders, Flowable<ByteBuffer>> response,
    ...
    public FileDownloadHeaders headers() {
    ...
    public RestResponse<FileDownloadHeaders, Flowable<ByteBuffer>> rawResponse() {
