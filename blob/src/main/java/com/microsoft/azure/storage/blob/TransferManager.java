...
public final class TransferManager {
    ...
    public static Single<BlockBlobCommitBlockListResponse> uploadFromNonReplayableFlowable(
        ...
        /*
        Write each buffer from the chunkedSource to the pool and call flush at the end to get the last bits. It is
        important that maxConcurrency=1 here as this enforces that data is read from the source sequentially and
        emitted from the resultant flowable sequentially.
         */
        return chunkedSource.flatMap(pool::write, false, 1)
                ...
                .concatMapEager(buffer -> {
                    ...
                    return blockBlobURL.stageBlock(blockId, data,
                            ...
                            .map(x -> {
                                pool.returnBuffer(buffer); // See notes on pool for why this is important.
                                return blockId;
                            }).toFlowable();
